...
if (!function_exists('amount_format')) {
	...
	{
		$currency = (object) get_option('base_currency', false);
		if ($icon_type == 'name') {
...
if (!function_exists('get_option')) {
	...
	{
		$local = session()->get('locale', env('DEFAULT_LANG'));
		$cacheKey = $withLocale ? $key . '_' . $local : $key;

		$option = cache_remember($cacheKey, function () use ($key, $withLocale, $local) {
			$query = Option::query()
				->where('key', $key);
			if ($withLocale) {
				$query = $query->where('lang', $local);
			}
...
if (!function_exists('activePlatforms')) {
	function activePlatforms()
	{
		return collect(config('platform'))->flatMap(fn($key, $value) => [$value => boolval(config('platform.' . $value . '.app_id'))]);
	}
}
...
if (!function_exists('getTranslationFile')) {
	function getTranslationFile()
	{
		...
		return file_get_contents(lang_path('/default.json'));
	}
}
