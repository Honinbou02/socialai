...
<template>
  <Link :href="route('user.publish', content.uuid)"
    class="flex h-96 min-w-[18rem] flex-col rounded-2xl border border-gray-50 shadow-md dark:border-gray-600 dark:bg-[#0f172a7e]">
  <div class="flex h-[17rem] w-full items-center justify-center overflow-hidden rounded-2xl">
    <img v-if="content.media.type === 'image' && content.media.url" v-lazy="content.media.url" alt="media" />
    <i v-else-if="content.media.type === 'image' && content.media.url === null"
      class="bx bx-image text-9xl text-secondary-700"></i>
    <i v-else-if="content.media.type === 'video'" class="bx bx-video text-9xl text-secondary-700"></i>
  ...
  <div class="border-y border-gray-200 px-4 py-2 dark:border-gray-700">
    <div class="-mb-2 flex items-center justify-between">
      <p class="font-semibold">{{ textExcerpt(content.title, 24) }}</p>
  ...
  <div class="flex items-center justify-between px-4 pt-3">
    <div class="flex h-full items-center gap-1">
      <template v-for="brandPlatform in content.platforms ?? []" :key="brandPlatform.platform">
        <img v-if="['published', 'failed'].includes(brandPlatform.status)" class="w-6 rounded-md border p-1" :class="{
          ...
          :title="brandPlatform.published_at ? 'published ' + moment(brandPlatform.published_at).fromNow() : brandPlatform.data?.message?.message ?? brandPlatform.data?.message" />
      </template>
    </div>
    <div class="badge scale-90 border dark:bg-secondary-800" :class="{
