...
const OverviewGrid = defineAsyncComponent(() => import('../../Components/Admin/OverviewGrid.vue'))
</script>
    ...
    <template v-if="segments.length">
      <div class="space-y-1 text-right">
        <template v-for="button in buttons" :key="button.index">
          <Link v-if="button.url != '#' && button.url" :href="button.url || '#'" class="btn btn-sm btn-primary mx-2">
          <div v-html="button.name"></div>
          </Link>
          <button v-else @click="(e) => e.preventDefault()" data-toggle="drawer" :data-target="button.target"
