...
<template>
  <div class="sidebar__wrapper">
    <div class="mb-40 sidebar__widget">
    ...
    <div class="mb-40 sidebar__widget" v-if="posts.length > 0">
      <div class="sidebar__widge-title-box">
        <h3 class="sidebar__widget-title">{{ trans("Recent Post") }}</h3>
      </div>
      <div class="sidebar__widget-content">
        <div class="sidebar__post rc__post">
          <div v-for="item in posts" class="mb-20 rc__post d-flex">
            <div class="mr-20 rc__post-thumb">
              <Link :href="route('blogs.show', item.slug)"
                ><img :src="item.preview?.value" alt=""
              /></Link>
            </div>
            <div class="rc__post-content">
              <h3 class="rc__post-title">
                <Link :href="route('blogs.show', item.slug)">{{ item.title }}</Link>
              </h3>
              <div class="rc__meta">
    ...
    <div class="mb-40 sidebar__widget" v-if="categories.length > 0">
      <div class="sidebar__widge-title-box">
    ...
    <div class="mb-40 sidebar__widget" v-if="tags.length > 0">
      <div class="sidebar__widge-title-box">
        <h3 class="sidebar__widget-title">{{ trans("Popular Tags") }}</h3>
      </div>
      <div class="sidebar__widget-content">
        <div class="tagcloud">
          <Link v-for="tag in tags" :href="`/blogs/tag/${tag.slug}`">
            {{ tag.title }}</Link
          >
        </div>
      </div>
