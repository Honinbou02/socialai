      ...
      <div class="row align-items-end">
        <div class="col-md-9">
          <div class="blog-details-banner z-index-2">
            <div class="blog-details-title-box">
              <span>{{
                blog.categories?.[0]?.title || trans("Uncategorized")
              }}</span>
        ...
        <div class="col-md-3 mb-15">
          <div class="blog-details-social-box z-index-3 text-md-end text-start">
            ...
            <a :href="socialShare('linkedin')"
              ><i class="fab fa-linkedin"></i
            ></a>
            <a :href="socialShare('instagram')"
      ...
      <div class="row">
        <div class="col-xl-12">
          <div class="blog-details-big-img z-index-2">
            <img
              :src="blog.banner?.value ?? blog.preview?.value"
              alt=""
              class="max-h-[400px]"
            />
          </div>
          ...
          <div class="postbox__details-wrapper pr-20">
            <article>
              ...
              <div class="postbox__details tagcloud mb-50">
                ...
                <Link v-for="tag in blog.tags" :href="`/blogs/tag/${tag.slug}`">
                  {{ tag.title }}</Link
                >
              </div>
              <div
                class="postbox__navigation-more mb-70 d-flex justify-content-between"
              >
                <div class="postbox__navigation-left d-flex align-items-center">
                  <div class="postbox__navigation-img rc__post-thumb">
                    <img
                      v-if="nextBlog"
                      :src="nextBlog.preview?.value"
                      alt=""
                    />
                  </div>
                  <div v-if="nextBlog" class="postbox__navigation-content">
                    ...
                    <h5>
                      <Link v-if="nextBlog" :href="`/blogs/${nextBlog.slug}`">{{
                ...
                <div
                  v-if="prevBlog"
                  class="postbox__navigation-right d-flex align-items-center"
                >
                  <div class="postbox__navigation-content">
                    ...
                    <h5>
                      <Link :href="`/blogs/${prevBlog.slug}`">{{
                        textExcerpt(prevBlog.title, 20)
                      }}</Link>
                    </h5>
                  </div>
                  <div class="postbox__navigation-img rc__post-thumb">
                    <img :src="prevBlog?.preview?.value" class="" alt="" />
                  </div>
                </div>
              </div>
            </article>
                ...
                <div class="tp-blog-item">
                  <div class="tp-blog-thumb fix">
                    <Link :href="route('blogs.show', blog)"
                      ><img class="w-100" :src="blog.preview?.value" alt=""
                    /></Link>
                  </div>
