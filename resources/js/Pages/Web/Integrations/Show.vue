          ...
          <div class="sv-details-widget">
            <div class="sv-details-category mb-30">
              ...
              <div class="sv-details-category-list">
                <ul>
                  <li v-for="item in sidebarIntegrations" :class="{ active: integration.id == item.id }">
                    <Link :href="`/integrations/${item.slug}`"><span>{{ textExcerpt(item.title, 18) }}</span>
                    <i class="fal fa-angle-right"></i></Link>
                  </li>
                </ul>
              </div>
            </div>
            <div class="tp-service__dashboard mb-30" :style="{ 'background-image': `url(${sidebar_card.bg_img})` }">
              <div class="tp-service__top-content">
                <h3 class="tp-service__title-white" v-html="sidebar_card.title"></h3>
                <p v-html="sidebar_card.sub_title"></p>
                <a class="tp-btn-orange tp-btn-hover alt-color-white" :href="sidebar_card.btn_link ?? '#'">
                  <span>{{ sidebar_card.btn_text }}</span>
                  <b></b>
                </a>
              </div>
              <div class="tp-service__dashdboard-sm-img">
                <img class="wow tpfadeRight" data-wow-duration=".9s" data-wow-delay=".3s" :src="sidebar_card.bottom_img"
                  alt="" />
              </div>
            </div>
        ...
        <div class="col-xl-8 col-lg-8">
          <div class="sv-details-wrapper">
            <div class="sv-details-thumb mb-45">
              <img class="w-100" :src="integration.preview_2" alt="" />
            </div>

            <div v-html="integration.long_description"></div>
          </div>
        </div>
      </div>
    </div>
