  ...
  <main class="container flex-grow p-4 sm:p-6">
    <div class="mt-4 flex items-center justify-between">
      <div class="card max-w-max p-1">
        <button class="btn w-full px-12 py-2 md:w-auto" :class="{ 'btn-primary': activeTab === 'calendar' }"
          @click="activeTab = 'calendar'">
          <Icon icon="fe:calendar" height="1rem" width="1rem"></Icon>
          <span>{{ trans('Calender') }}</span>
        </button>
        <button class="btn w-full px-12 py-2 md:w-auto" :class="{ 'btn-primary': activeTab === 'card' }"
      ...
      <div class="card" v-if="activeTab === 'calendar'">
        ...
        <div v-else class="card-body">
          ...
          <div class="mt-5 flex gap-3">
            <button @click="startFromDate(moment(weekStartDate).days(-7).format('Y-M-D'))"
            ...
            <button @click="startFromDate(moment(weekEndDate).days(1).format('Y-M-D'))"
              class="btn rounded-lg border-dashed bg-slate-100 px-2 py-1 hover:bg-slate-200 dark:bg-slate-900 dark:hover:bg-gray-700">
          ...
          <div class="mt-5 flex flex-wrap items-start justify-evenly gap-2">
            <div
              ...
              v-for="date in items" :key="date.date">
              <p class="mb-3">{{ date.date }}</p>
              <button @click="modal.open('createContent')"
                class="btn whitespace-nowrap border border-dashed border-green-500 text-green-900 hover:bg-gray-700 dark:text-white">
                {{ trans('Create New') }}
              </button>
              <div v-if="date.contents?.length" class="mt-2 rounded-lg bg-white p-1 text-sm dark:bg-slate-700">
                <div v-for="post in date.contents" :key="post.id"
                  class="my-3 flex flex-col items-center justify-center rounded-lg border p-2 text-center dark:border-slate-500">
                  <Link :href="post?.brandpost?.uuid ? route('user.publish', post?.brandpost?.uuid) : '#'
                    ">
                  ...
                  <!-- remove button -->
                  <button @click="deleteRow(route('user.brand-post-platforms.removeSchedule', post))"
