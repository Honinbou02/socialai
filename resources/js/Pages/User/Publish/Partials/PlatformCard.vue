  ...
  <div class="m-auto max-w-md" :class="{ group: !optionsAreaState }">
    <div class="card flex items-center rounded-none rounded-t-xl px-5 py-3">
      ...
      <div class="ml-4">
        ...
        <span class="text-xs">{{ trans('10h Ago') }}</span>
      </div>
    </div>
    <p
    ...
    >
      ...
      >
        <p class="text-lg font-semibold">{{ trans('Rendering...') }}</p>
    ...
    >
      ...
      <small>{{ trans('12 comments 2 shares') }}</small>
    </div>
    <div
    ...
    >
      <label class="label mb-2 font-semibold"
        >{{ trans('Who can view this video') }} <span class="text-red-600">{{ trans('(required)') }}</span>
      </label>
      <select class="select" v-model="store.tiktokOptions.privacy_level">
        <option value="" disabled>{{ trans('Select Privacy') }}</option>
      ...
      <div class="mt-3">
        <label class="label mb-2 font-semibold">{{ trans('Disable (optional)') }}</label>
        <div class="flex">
          <div class="flex flex-1 items-center gap-2">
            ...
            />
            <label class="label" for="comment"> {{ trans('Comment') }} </label>
          </div>
          <template v-if="store.getActivePlatformContent('media_type') == 'video'">
            <div class="flex flex-1 items-center gap-2">
              ...
              />
              <label class="label" for="duet"> {{ trans('Duet') }} </label>
            </div>
            <div class="flex flex-1 items-center gap-2">
              <input
                type="checkbox"
              ...
              />
              <label class="label" for="stitch"> {{ trans('Stitch') }} </label>
            </div>
          </template>
        </div>
      </div>
