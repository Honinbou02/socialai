...
const submit = (gateway_id) => {
  if (findGateway.value.min_amount > convertedAmount.value) {
    notify.danger(
      trans('The minimum transaction amount is ') + formatCurrency(findGateway.value.min_amount)
    )
    return
  }
  if (findGateway.value.max_amount != -1) {
    if (findGateway.value.max_amount < convertedAmount.value) {
      notify.danger(
        trans('The maximum transaction amount is ') + formatCurrency(findGateway.value.max_amount)
      )
      return
    }
  }
...
<template>
            ...
            <div class="mt-2">
              ...
              >
                <span class="btn-label-wrap"
                  ><span class="btn-label-inner">{{ trans('Increase limit‍') }}</span></span
                >
              </button>
            </div>
          </div>
  ...
  <Modal :state="getState('addCredits')" :header-state="true" header-title="Buy Credits">
    ...
    <div class="my-8 flex gap-2 px-3">
