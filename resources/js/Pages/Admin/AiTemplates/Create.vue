...
const props = defineProps([
  ...
  'image_sizes',
  'instructions',
  'STABLE_DIFFUSION_API_KEY_EXISTS'
])
...
const list = [
  ...
  },
  {
    title: 'video',
    icon: 'bxs-videos',
    visible: props.STABLE_DIFFUSION_API_KEY_EXISTS
  },
  {
    title: 'audio',
...
<template>
  <main class="container flex-grow p-4 sm:p-6">
    <PageHeader :title="trans('Create Template')" :buttons="buttons" />

    <div class="mx-auto lg:w-9/12">
      <h3 class="card-title mb-3">{{ trans('Create New Template') }}</h3>
      ...
      <form @submit.prevent="submit" class="space-y-7">
        <div class="mt-2 grid grid-cols-2 gap-4 lg:grid-cols-5">
          <template v-for="item in list" :key="item.title">
            ...
            >
              ...
              <span class="mb-1">{{ trim(item.title) }}</span>
            </button>
          </template>
        </div>
        <div class="card card-body">
          ...
          <div class="flex gap-4">
            ...
            <div class="flex-1" v-if="form.prompt_type == 'image'">
              <label class="label mb-1">{{ trans('Ai Model') }}*</label>
              <select
                required
                class="select capitalize"
                @change="form.reset('meta')"
        ...
        <!-- text -->
        <div class="card card-body grid grid-cols-3 gap-4" v-if="form.prompt_type == 'text'">
          <div>
            <label class="label mb-1">{{ trans('Ai Model') }}*</label>
        ...
        >
          ...
          <div>
            ...
            <small>
              <a :href="instructions[form.prompt_type]" target="_blank">
                * {{ trans('instructions') }}
              </a>
            </small>
          </div>
